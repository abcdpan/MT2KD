...
class Classification(nn.Module):
    def __init__(self, channels=256,num_classes=100,size=4):
        super(Classification, self).__init__()
        self.channels = channels
        self.num_classes = num_classes
        self.avgpool_1 = nn.AdaptiveAvgPool2d((size, size))
        self.fc1 = nn.Linear(channels*size*size, self.num_classes)
    def _initialize_weights(self):
        """自定义凯明初始化函数"""
        for m in self.modules():
            # 对线性层应用凯明初始化
            if isinstance(m, nn.Linear):
                # 凯明正态分布初始化（默认针对ReLU激活）
